...
# Creating a sampler
betas <- matrix(0,nrow = nmcmc, ncol = NCOL(x))
...
for(i in 1:length(selected_vars)){
  sigma_likelihood <- sigma_likelihood + (tau_beta_j[selected_vars[i]]^-1)*tcrossprod(x[,selected_vars[i],drop = FALSE])
}
...
for(i in 1:length(new_selected_vars)){
  new_sigma_likelihood <- new_sigma_likelihood + (tau_beta_j[new_selected_vars[i]]^-1)*tcrossprod(x[,new_selected_vars[i],drop = FALSE])
}
...
for(ii in 1:length(selected_vars)){
  ...
  if(length(selected_vars)>1){
    ...
    residuals_  <- x[,other_pred,drop = FALSE] %*% betas[mcmc_iter,other_pred]
  } else {
    residuals_ <- rep(0,n)
  }
  betas[mcmc_iter,selected_vars[ii]] <- mvnfast::rmvn(n = 1,mu = (crossprod(x[,selected_vars[ii], drop = FALSE]) + tau_beta_j[ii]*tau^-1)^-1 *
                  crossprod(x[,selected_vars[ii], drop = FALSE],(y - residuals_)), sigma = (tau^(-1))*(crossprod(x[,selected_vars[ii], drop = FALSE]) + tau_beta_j[ii]*tau^-1)^-1 )
